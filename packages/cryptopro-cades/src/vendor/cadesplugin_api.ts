// КриптоПро ЭЦП Browser plug-in 2.0.15400 для Windows и Unix-платформ от 09.04.2025.
// https://t.me/csp_beta/15
// @ts-nocheck
export default function init() {
    ...
    window.addEventListener(
      ...
      function (event) {
        ...
        if (
          ...
          event.data.indexOf(resp_prefix) !== 0
        ) {
          return;
        }
  ...
  function async_spawn(generatorFunc) {
    function continuer(verb, arg) {
      ...
      if (result.done) {
        ...
        return window.Promise.resolve(result.value).then(
          onFulfilled,
          onRejected,
        );
      }
    ...
    call: function call(functionName, args, callback) {
      ...
      try {
        iframe.setAttribute(
          ...
          callbackId +
          ':' +
          encodeURIComponent(window.JSON.stringify(args, arrObjs)),
        );
      } catch (e) {
        window.alert(e);
      }
  ...
  function show_firefox_missing_extension_dialog() {
    if (!window.cadesplugin_skip_extension_install) {
      ...
      ovr.style =
        'visibility: hidden; position: fixed; left: 0; top: 0; width:100%; height:100%; background-color: rgba(0,0,0,0.7)';

      ovr.innerHTML =
        "<div id='cadesplugin_ovr_item' style='position:relative; max-width:400px; margin:100px auto; background-color:#fff; border:2px solid #000; padding:10px; text-align:center; opacity: 1; z-index: 1500'>" +
  ...
  function is_capilite_enabled() {
    // noinspection JSUnresolvedVariable
    return (
      typeof cadesplugin.EnableInternalCSP !== 'undefined' &&
      cadesplugin.EnableInternalCSP
    );
  }
